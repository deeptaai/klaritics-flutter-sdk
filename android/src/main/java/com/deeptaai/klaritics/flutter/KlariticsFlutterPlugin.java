...
import static com.deeptaai.klaritics.Constants.ADDITIONAL_INFO;
...
import com.deeptaai.klaritics.AnthraDataCallback;
...
import com.deeptaai.klaritics.utils.BidiEvents;
import com.deeptaai.klaritics.utils.Logger;
import com.deeptaai.klaritics.utils.Receiver;
...
public class KlariticsFlutterPlugin implements FlutterPlugin, MethodCallHandler, EventListener {
  ...
  public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
    ...
    channel = new MethodChannel(flutterPluginBinding.getBinaryMessenger(), "plugins.flutter.io/klaritics_flutter");
    channel.setMethodCallHandler(this);
    commandChannel = new BasicMessageChannel<>(
            flutterPluginBinding.getBinaryMessenger(),
            "plugins.flutter.io/klaritics_commands",
  ...
  private void registerWithSdk() {
    final SDKController controller = SDKController.getInstance();
    controller.markAsFlutter();
    controller.registerToEvent(INTERNAL_EVENTS, this);

    id = Klaritics.getDeviceId(applicationContext);
  ...
  private void handleSetup(MethodCall call, Result result) {
    String appId = call.argument("appId");
    String host = call.argument("host");
    if (appId == null || appId.isEmpty() || host == null || host.isEmpty()) {
      result.error("Klaritics", "Klaritics cannot be initialized: `appId` and `host` are required in setup", null);
      return;
    }
    Klaritics.setup(applicationContext, new KlariticsConfig(appId, host));
    registerWithSdk();
    result.success(null);
  }
  ...
  private void registerEmbedView(@NonNull FlutterPluginBinding flutterPluginBinding) {
        ...
        .registerViewFactory("com.deeptaai.klaritics.flutter/KlariticsEmbedView", new KlariticsEmbedViewFactory(
                flutterPluginBinding.getBinaryMessenger()));
  }
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    switch (call.method) {
      ...
      case "setCurrentScreenName":
        ...
        break;
      case "getDeviceId":
        result.success(Klaritics.getDeviceId(SDKController.getInstance().getContext()));
        break;
      case "getAttributes":
      ...
      default:
        if (call.method.equals(ES)) {
          ...
          result.error("Klaritics", "Unknown method " + call.method, null);
        }
    }
  }
  ...
  private void handleMethodCall(MethodCall call, Result result, String name) {
    ...
    try {
      ...
      switch (name) {
        case "App":
          Klaritics.logAppEvent(eventName, attributes);
          break;
        case "Client":
          Klaritics.logClientEvent(eventName, attributes);
          break;
        case "User":
          Klaritics.setUserCustomInfo(attributes);
          break;
        case "Session":
          Klaritics.setSessionCustomInfo(attributes);
          break;
        case "Internal":
      ...
      result.error("Klaritics", "Failed to parse attributes in log" + name +"Event. " + e.getMessage(), null);
    }
  }
  ...
  private void handleSetUserIdentifier(MethodCall call, Result result) {
    ...
    Klaritics.setUserIdentifier(customUserId);
    result.success(null);
  }
  ...
  private void handleGetAttributes(MethodCall call, Result result) {
    ...
    if (attributes != null) {
      ...
      Klaritics.getAttributes(attrsArray, new AnthraDataCallback() {
        @Override
        ...
        public void onFailure() {
          mainThread.post(() -> result.error("Klaritics", "Failed to get attributes", null));
        }
      });
    }
  }
  ...
  // NOTE: realtime-UI (in-app messages) is not yet available in klaritics-android-sdk 1.0.0.
  // This is a no-op until a Klaritics realtime-UI plugin ships. See KlariticsEmbedView.
  private void handleBB() {
    // TODO(klaritics): re-enable via UIManager.getInstance().removeMessage("", null)
    //  once the Klaritics realtime-UI plugin is available.
  }
